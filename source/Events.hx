...
class Events {
	...
	public static function obtainEvents() {
		loadedModules.clear();
		eventList = [];
		var tempEventArray:Array<String> = Util.readDirectory('assets/events');
		//
		...
		for (event in tempEventArray) {
			if (event.contains('.')) {
				...
				loadedModules.set(event, ScriptHandler.loadModule(event));
				futureEvents.push(event);
			} else {
				if (PlayState.SONG != null && CoolUtil.spaceToDash(PlayState.SONG.song.toLowerCase()) == event) {
					var internalEvents:Array<String> = Util.readDirectory(event);
					for (subEvent in internalEvents)
					{
						subEvent = subEvent.substring(0, subEvent.indexOf('.', 0));
						loadedModules.set(subEvent, ScriptHandler.loadModule(subEvent));
